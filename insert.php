<?php
   ...
   if(empty($name)) {
	 ...
	 exit;
	   ...
	   $query = "insert into freeboard(name, passwd, subject, content, regist_day, hit) values('$name', '$passwd', '$subject', '$content',NOW(), 0)";// 레코드 삽입 명령
	   
	   mysqli_query($dbc, $query);  // $query 에 저장된 명령 실행

	   mysqli_close($dbc);                // DB 연결 끊기
	   
	   $home_url = 'http://' . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . '/board.php';
	   header('Location:'.$home_url);  // list.php 로 이동합니다.
   }
?>
