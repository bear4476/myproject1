<?php
   ...
   $query = "select passwd from freeboard where board_id=$num";
   ...
   if ($passwd != $row['passwd'] && $_SESSION['username']!="admin"){
      ...
      $query = "delete from freeboard where board_id = $num";
      mysqli_query($dbc, $query);

      mysqli_close($dbc);
   }
	 $home_url = 'http://' . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . '/board.php?num='.$num.'&amp;page='.$page.'';
